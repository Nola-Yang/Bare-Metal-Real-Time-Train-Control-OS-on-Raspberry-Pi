...
CFLAGS:=-g -pipe -static -march=armv8-a -mcpu=cortex-a72 -mstrict-align -mgeneral-regs-only -fstack-protector-strong $(WARNINGS) -I$(INCLUDE_DIR) -I$(INCLUDE_DIR)/kernel -I$(INCLUDE_DIR)/user

# -Wl,option tells gcc to pass 'option' to the linker with commas replaced by spaces
LDFLAGS:=-Wl,-nmagic -Wl,-Tlinker.ld -Wl,--no-warn-rwx-segments -nostdlib -lgcc
