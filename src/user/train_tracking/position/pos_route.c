...
static void pos_apply_route_switches_forced(const int *sw_nums,
                                            const char *sw_dirs,
                                            int sw_count) {
...
static int pos_try_launch_committed_route_force(train_pos_t *pos,
                                                uint64_t now_us) {
    int start_cursor = 0;
    int did_initial_reverse;
    track_node *cur_sensor_orig;
    ...
    pos_apply_route_switches_forced(g_pos_try_reserve_plan.sw_nums,
...
int pos_try_direct_goto_force_reachable(train_pos_t *pos) {
    ...
    if (pos_evaluate_target_plan(pos, user_target, &g_pos_try_eval_main) !=
