...
void can_data_get_sensor(CanData *can_data, SensorData *sensor_data) {
    ...
    uint16_t sensor_id = ((uint16_t)data[2] << 8) | data[3];
...
void can_data_log(CanData *can_data, uint32_t line_no) {
    for (uint32_t i = 0; i < 4; ++i) {
        uart_debug_move_cursor(CONSOLE, line_no, 1);
        uart_debug_printf(CONSOLE, "CAN --> ID[%u]: %x", i, can_data->id[i]);
        line_no++;
    }

    uint8_t data_len = can_data->length;

    uart_debug_move_cursor(CONSOLE, line_no, 1);
    uart_debug_printf(CONSOLE, "CAN --> DLC: %x", data_len);
    line_no++;

    for (uint32_t i = 0; i < data_len; ++i) {
        uart_debug_move_cursor(CONSOLE, line_no, 1);
        uart_debug_printf(CONSOLE, "CAN --> DATA[%u]: %x", i, can_data->data[i]);
        line_no++;
    }
}